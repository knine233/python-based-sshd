...
class ModelWrapper(nn.Module):
    ...
    def snapshot(self, it, optimizer, args, finished=False):
        """
        Save model snapshot.
        ...
        """
        if finished:
            result_name = args.t_name
            torch.save({
            'iteration': it,
            'model_state_dict': self.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
        }, os.path.join('checkpoints', result_name))
    ...
    def load_snapshot(self, args, root, optimizer=None):
        """
        Load model snapshot.
        ...
        """
        checkpoint = torch.load(root + args.t_name)
