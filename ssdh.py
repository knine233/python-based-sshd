...
def train(train_dataloader,
    ...
    features = extract_features(model, train_dataloader, num_features, args.device, args.verbose)

    # Generate similarity matrix
    if args.distance_measure == 'cos':
        S = generate_cos_similarity_matrix(features, args.alpha, args.beta).to(args.device)
    elif args.distance_measure == "DU3H":
        S = generate_cos_similarity_matrix(features, args.alpha, args.beta, True).to(args.device)
    else:
        raise TypeError("Invalid distance measure method")
    # Training
    model.train()
    for epoch in range(resume_it, args.max_iter):
        ...
        for i, (data, _, index) in enumerate(train_dataloader):
            ...
            # Evaluate
            if cur_iter % args.evaluate_interval == 0:
                mAP = evaluate(model,
                               query_dataloader,
                               retrieval_dataloader,
                               args.code_length,
            ...
            # Save snapshot
            if cur_iter % args.snapshot_interval == args.snapshot_interval - 1:
                model.snapshot(cur_iter, optimizer, args)
    ...
    model.snapshot(cur_iter, optimizer, args, finished=True)
...
# def generate_BD_similarity_matrix():


def generate_cos_similarity_matrix(features, alpha, beta, is_DH3U=False):
    """
    Generate similarity matrix.
    ...
    ds = left_mean - alpha * left_std       # 接近阈值
    dd = right_mean + beta * right_std      # 远离阈值
    S = (cos_dist < ds) * 1.0 + (cos_dist > dd) * -1.0
    if is_DH3U:
        S += (1-cos_dist) * (1-cos_dist) * (ds < cos_dist) * (cos_dist < dd)
    return torch.FloatTensor(S)


def extract_features(model, dataloader, num_features, device, verbose):
    """
    Extract features from fc7 of VGG-F.
